...
def write_diff_flux_to_file(g_chi, m_chi, nucleus, m_n, J_n, dE, GT, b_min, b_max, delta_b, l_min1, l_max1, l_min2, l_max2, delta_l, R_max):
    ...
    ****************
    ...
    for l in [*ells1, *ells2]:
        ...
        # Path("data_no_conv/new_baryon_dispersion/differential_flux_data_by_dE/{}/dE_{}MeV/m_chi_{}MeV".format(nucleus, dE*energyScale, m_chi*energyScale)).mkdir(parents=True, exist_ok=False)
        # filename = 'data_no_conv/new_baryon_dispersion/differential_flux_data_by_dE/{}/dE_{}MeV/m_chi_{}MeV/l_{}deg.txt'.format(nucleus, dE*energyScale, m_chi*energyScale, l)
...
def main_no_conv(num_cores):
    ...
    print(nucleus)
    for dE in nuc_dict[nucleus]['dEs [MeV]'][23:max_dict[nucleus]:4]:
        try:
...
# run example:
# python3 fullFluxCalculation_unitless_multiprocessing.py --num_cores 16
